...
export default function () {
  ...
  const sendRes = http.post(`${BASE}/api/v1/chat/conversations/${convId}/messages`, JSON.stringify({ userId, content: 'hello' }), {
    headers: { 'Content-Type': 'application/json' },
    tags: { endpoint: 'send' },
  });
  check(sendRes, { 'send status is 201|200': (r) => r.status === 201 || r.status === 200 });

  const historyRes = http.get(`${BASE}/api/v1/chat/conversations/${convId}/messages?limit=20` , {
