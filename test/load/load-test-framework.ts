...
export class LoadTestRunner {
  ...
  private static evaluateSuccess(
    ...
    const checks = {
      ...
      poolSaturation: !metrics.poolSaturation || metrics.poolSaturation < 0.8, // <80%
      cacheHitRatio: !metrics.cacheHitRatio || metrics.cacheHitRatio > 0.7, // >70%
  ...
  private static printResults(
    ...
    console.log(
      `  ${result.throughput >= config.expectedThroughput ? "✅" : "❌"} ${result.throughput >= config.expectedThroughput ? "PASS" : "FAIL"}`,
    );
    console.log("");
    console.log("Latency:");
    ...
    console.log(
      `  P95: ${result.latency.p95.toFixed(2)}ms (target: ${config.expectedP95Ms}ms)`,
    );
    console.log(`  P99: ${result.latency.p99.toFixed(2)}ms`);
    console.log(`  Max: ${result.latency.max.toFixed(2)}ms`);
    console.log(
      `  ${result.latency.p95 <= config.expectedP95Ms ? "✅" : "❌"} ${result.latency.p95 <= config.expectedP95Ms ? "PASS" : "FAIL"}`,
    );
    console.log("");
    ...
    if (Object.keys(result.metrics).length > 0) {
      ...
      if (result.metrics.poolSaturation !== undefined) {
        console.log(
          `  Pool Saturation: ${(result.metrics.poolSaturation * 100).toFixed(1)}%`,
        );
        console.log(
          `    ${result.metrics.poolSaturation < 0.8 ? "✅" : "❌"} ${result.metrics.poolSaturation < 0.8 ? "PASS" : "FAIL"} (target: <80%)`,
        );
      }
      if (result.metrics.cacheHitRatio !== undefined) {
        console.log(
          `  Cache Hit Ratio: ${(result.metrics.cacheHitRatio * 100).toFixed(1)}%`,
        );
        console.log(
          `    ${result.metrics.cacheHitRatio > 0.7 ? "✅" : "❌"} ${result.metrics.cacheHitRatio > 0.7 ? "PASS" : "FAIL"} (target: >70%)`,
        );
      }
      if (result.metrics.replicaLag !== undefined) {
        console.log(`  Replica Lag: ${result.metrics.replicaLag.toFixed(2)}s`);
        console.log(
          `    ${result.metrics.replicaLag < 5 ? "✅" : "❌"} ${result.metrics.replicaLag < 5 ? "PASS" : "FAIL"} (target: <5s)`,
        );
      }
      console.log("");
    }
