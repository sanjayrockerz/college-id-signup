...
describe('Data Layer Integration Tests', () => {
  ...
  describe('EXPLAIN Plan Validation', () => {
    ...
    it('should use composite index for message history with keyset pagination', async () => {
      ...
      const cursor = KeysetPaginator.encodeCursor(new Date(), 'test-message-id');
      ...
      expect(['Index Scan', 'Index Only Scan']).toContain(plan.Plan['Node Type']);
    });
    ...
    it('should use efficient join for conversation list with participants', async () => {
      ...
      const planStr = JSON.stringify(plan);
      expect(planStr).toMatch(/idx_conversation_participant|idx_conversation_updated/);
  ...
  describe('Cache Validity and Invalidation', () => {
    ...
    it('should cache and retrieve data correctly', async () => {
      const testData = { id: testUserId, name: 'Test User', email: 'test@example.com' };
    ...
    it('should handle cache invalidation patterns correctly', async () => {
      ...
      await cache.set(`conversation:${conversationId}`, { id: conversationId }, 60);
      await cache.set(`user:${userId}:conversations`, ['conv1', conversationId], 60);
