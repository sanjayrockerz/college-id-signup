...
class SyntheticDataLoader {
  ...
  constructor(schemaName: string, configPath: string) {
    ...
    if (process.env.NODE_ENV === "production") {
      throw new Error("FATAL: Cannot run synthetic data loader on production environment");
    }
    ...
    console.log(`[Loader] Dataset: band=${this.config.band}, seed=${this.config.seed}`);
  }
  ...
  async verifyIntegrity(): Promise<boolean> {
    ...
    try {
      ...
      if (orphanedConversations[0]?.count > 0) {
        console.error(`[Integrity] ✗ Found ${orphanedConversations[0].count} orphaned conversations`);
        return false;
      }
      ...
      if (orphanedMessages[0]?.count > 0) {
        console.error(`[Integrity] ✗ Found ${orphanedMessages[0].count} orphaned messages`);
        return false;
      }
      ...
      if (orphanedMembers[0]?.count > 0) {
        console.error(`[Integrity] ✗ Found ${orphanedMembers[0].count} orphaned conversation members`);
        return false;
      }

      console.log("[Integrity] ✓ All referential integrity constraints satisfied");
      return true;
    } catch (error: any) {
  ...
  async countRows(): Promise<void> {
    ...
    this.metadata.rows_loaded.conversations = await this.prisma.conversation.count();
    this.metadata.rows_loaded.conversation_users = await this.prisma.conversationUser.count();
    this.metadata.rows_loaded.messages = await this.prisma.message.count();
    this.metadata.rows_loaded.attachments = await this.prisma.attachment.count();
    this.metadata.rows_loaded.message_reads = await this.prisma.messageRead.count();

    console.log("[Count] Row counts:");
    console.log(`  Users: ${this.metadata.rows_loaded.users.toLocaleString()}`);
    console.log(`  Conversations: ${this.metadata.rows_loaded.conversations.toLocaleString()}`);
    console.log(`  Conversation Users: ${this.metadata.rows_loaded.conversation_users.toLocaleString()}`);
    console.log(`  Messages: ${this.metadata.rows_loaded.messages.toLocaleString()}`);
    console.log(`  Attachments: ${this.metadata.rows_loaded.attachments.toLocaleString()}`);
    console.log(`  Message Reads: ${this.metadata.rows_loaded.message_reads.toLocaleString()}`);
  }
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.metadata.duration_seconds = (endTime.getTime() - this.startTime.getTime()) / 1000;
      this.metadata.status = "COMPLETED";

      console.log(`[Load] ✓ Load completed in ${this.metadata.duration_seconds}s`);
    } catch (error: any) {
  ...
  async saveMetadata(outputPath: string): Promise<void> {
    ...
    const summary = {
      ...
      total_rows: Object.values(this.metadata.rows_loaded).reduce((a, b) => a + b, 0),
...
async function main() {
  ...
  if (schemaIndex === -1 || configIndex === -1) {
    console.error("Usage: ts-node loader.ts --schema <schema_name> --config <config.json>");
    console.error("Example: ts-node loader.ts --schema perf_synthetic --config report_staging_*.json");
    process.exit(1);
  }
  ...
  if (process.env.NODE_ENV === "production") {
    console.error("FATAL: Cannot run on production. Set NODE_ENV=development or staging");
    process.exit(1);
  }
