...
class GeneratorCalibrator {
  ...
  constructor(metricsPath: string) {
    ...
    console.log(`[Calibrator] ✓ Loaded metrics from ${this.metrics.extracted_at}`);
    console.log(`[Calibrator]   Users: ${this.metrics.users.total_count.toLocaleString()}`);
    console.log(`[Calibrator]   Conversations: ${this.metrics.conversations.total_count.toLocaleString()}`);
    console.log(`[Calibrator]   Messages: ${this.metrics.messages.total_count.toLocaleString()}\n`);
  }

  /**
   * Estimate normal distribution parameters from histogram
   */
  private fitNormalFromHistogram(histogram: Record<string, number>): { mean: number; stddev: number } {
    let totalCount = 0;
    let weightedSum = 0;
  ...
  calibrate(): CalibratedConfig {
    ...
    console.log("[Calibrator] Fitting username length (normal distribution)...");
    const usernameParams = this.fitNormalFromHistogram(
      this.metrics.users.username_length_histogram
    );
    console.log(`[Calibrator]   Mean: ${usernameParams.mean.toFixed(2)}, StdDev: ${usernameParams.stddev.toFixed(2)}`);
    ...
    console.log(`[Calibrator]   DIRECT_MESSAGE: ${(convoTypesDist.DIRECT_MESSAGE * 100).toFixed(1)}%`);
    console.log(`[Calibrator]   GROUP_CHAT: ${(convoTypesDist.GROUP_CHAT * 100).toFixed(1)}%`);
    console.log(`[Calibrator]   CHANNEL: ${((convoTypesDist.CHANNEL || 0) * 100).toFixed(1)}%`);
    ...
    console.log(`[Calibrator]   Range: [${msgPerConvoParams.min}, ${msgPerConvoParams.max}]`);
    console.log(`[Calibrator]   Production p50: ${this.metrics.conversations.messages_per_conversation_percentiles.p50}`);
    console.log(`[Calibrator]   Production p99: ${this.metrics.conversations.messages_per_conversation_percentiles.p99}`);
    ...
    const hourlyWeights = this.normalizeWeights(this.metrics.messages.hourly_distribution);
    const dowWeights = this.normalizeWeights(this.metrics.messages.day_of_week_distribution);
    
    const peakHour = hourlyWeights.indexOf(Math.max(...hourlyWeights));
    const peakDay = dowWeights.indexOf(Math.max(...dowWeights));
    const dayNames = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
    console.log(`[Calibrator]   Peak hour: ${peakHour}:00 (${(hourlyWeights[peakHour] * 100).toFixed(1)}%)`);
    console.log(`[Calibrator]   Peak day: ${dayNames[peakDay]} (${(dowWeights[peakDay] * 100).toFixed(1)}%)`);
    ...
    const config: CalibratedConfig = {
      ...
      conversation_generation: {
        ...
        messages_per_conversation: {
          ...
          p50: this.metrics.conversations.messages_per_conversation_percentiles.p50,
          p90: this.metrics.conversations.messages_per_conversation_percentiles.p90,
          p99: this.metrics.conversations.messages_per_conversation_percentiles.p99,
        },
      },
      message_generation: {
        content_length: {
