...
async function quickGenerate() {
  ...
  const words = ["hello", "world", "test", "message", "chat", "team", "project", "update"];
  ...
  for (const convo of convos) {
    const memberCount = convo.type === "DIRECT_MESSAGE" ? 2 : rng.nextInt(3, 10);
    const selectedUsers = [];
    ...
    if (memberBatch.length >= 5000) {
      await prisma.conversationUser.createMany({ data: memberBatch, skipDuplicates: true });
  ...
  for (const convo of convosWithMembers) {
    ...
    for (let i = 0; i < msgsPerConvo && msgCount < targetMessages; i++) {
      const msgType = rng.next() < 0.85 ? "TEXT" : rng.next() < 0.7 ? "IMAGE" : "FILE";
      const content = Array(rng.nextInt(5, 15)).fill(0).map(() => rng.choice(words)).join(" ");
      const timestamp = new Date(baseTime + rng.nextInt(0, 20 * 24 * 3600 * 1000));
  ...
  // 7. Summary
  const finalCounts = {
    users: await prisma.user.count({ where: { email: { startsWith: "syn_" } } }),
