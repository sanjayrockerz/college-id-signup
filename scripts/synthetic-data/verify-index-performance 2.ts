...
class IndexPerformanceVerifier {
  ...
  async captureBaseline(): Promise<BaselineReport> {
    ...
    queries.push(await this.captureQuery(
      "Q1_message_history",
    ...
    // Generate summary
    const executionTimes = queries.map((q) => q.execution_time_ms).sort((a, b) => a - b);
    const p50 = this.percentile(executionTimes, 0.5);
    const p95 = this.percentile(executionTimes, 0.95);
    const avgTime = executionTimes.reduce((a, b) => a + b, 0) / executionTimes.length;
    const totalSorts = queries.filter((q) => q.plan_analysis.has_sort_node).length;
  ...
  private async captureQuery(
    ...
    console.log(`  Buffers: ${buffers.shared_hit} hit / ${buffers.shared_read} read`);
  ...
  private analyzePlan(plan: any): QueryPlan["plan_analysis"] {
    ...
    const estimationAccuracy =
      rowsEstimated > 0 ? Math.min(rowsActual / rowsEstimated, rowsEstimated / rowsActual) : 1;
  ...
  static async compareBaselines(
    ...
    const preBaseline: BaselineReport = JSON.parse(fs.readFileSync(preBaselinePath, "utf-8"));
    const postBaseline: BaselineReport = JSON.parse(fs.readFileSync(postBaselinePath, "utf-8"));
    ...
    for (const preQuery of preBaseline.queries) {
      const postQuery = postBaseline.queries.find((q) => q.query_id === preQuery.query_id);

      if (!postQuery) {
        console.log(`⚠️  Query ${preQuery.query_id} not found in post-optimization baseline`);
        continue;
      }

      const timeDelta = postQuery.execution_time_ms - preQuery.execution_time_ms;
      const timeReduction = -timeDelta / preQuery.execution_time_ms;
      const sortEliminated =
        preQuery.plan_analysis.has_sort_node && !postQuery.plan_analysis.has_sort_node;
      const indexAdopted =
        !preQuery.plan_analysis.uses_index && postQuery.plan_analysis.uses_index;

      let status: "IMPROVED" | "DEGRADED" | "NO_CHANGE";
      if (timeReduction > 0.10) {
        status = "IMPROVED";
      } else if (timeReduction < -0.10) {
        status = "DEGRADED";
      } else {
        status = "NO_CHANGE";
      }
      ...
      console.log(`  Δ:    ${timeDelta > 0 ? "+" : ""}${timeDelta.toFixed(2)}ms (${(timeReduction * 100).toFixed(1)}%)`);
      console.log(`  Sort: ${sortEliminated ? "ELIMINATED ✓" : preQuery.plan_analysis.has_sort_node ? "STILL PRESENT ⚠️" : "N/A"}`);
      console.log(`  Index: ${indexAdopted ? "ADOPTED ✓" : postQuery.plan_analysis.uses_index ? "USED" : "NO"}`);
      ...
      if (status === "DEGRADED") {
        mitigations.push({
          ...
          recommendation: "Review index maintenance overhead vs read benefit. Consider partial index.",
        });
      }

      if (postQuery.plan_analysis.has_sort_node && preQuery.query_name.includes("ORDER BY")) {
        mitigations.push({
          query_id: preQuery.query_id,
          issue: "Sort node still present despite index",
          recommendation: "Verify composite index covers ORDER BY column. Check index direction (ASC/DESC).",
        });
      }
      ...
      if (postQuery.buffers.shared_read > postQuery.buffers.shared_hit * 0.5) {
        mitigations.push({
          ...
          recommendation: "Consider covering index with INCLUDE clause to avoid heap lookups.",
        });
      }
    ...
    const avgImprovement =
      improvements.reduce((sum, i) => sum + i.time_reduction_pct, 0) / improvements.length;
    const p95Improvement =
      ((preBaseline.summary.p95_execution_time_ms - postBaseline.summary.p95_execution_time_ms) /
        preBaseline.summary.p95_execution_time_ms) *
      100;

    const queriesImproved = improvements.filter((i) => i.status === "IMPROVED").length;
    const queriesDegraded = improvements.filter((i) => i.status === "DEGRADED").length;
    const queriesUnchanged = improvements.filter((i) => i.status === "NO_CHANGE").length;
    const sortsEliminated = improvements.filter((i) => i.sort_eliminated).length;
  ...
  private serializeParameters(params: any[]): Record<string, any> {
    ...
    params.forEach((param, idx) => {
      result[`$${idx + 1}`] = typeof param === "string" ? `<id:${param.substring(0, 8)}...>` : param;
    });
    return result;
  }
...
async function main() {
  ...
  if (args.includes("--compare")) {
    ...
    if (!preBaseline || !postBaseline) {
      console.error("Usage: ts-node verify-index-performance.ts --compare <pre_baseline.json> <post_baseline.json>");
      process.exit(1);
    }

    const report = await IndexPerformanceVerifier.compareBaselines(preBaseline, postBaseline);
    ...
    console.log("\n=== COMPARISON SUMMARY ===");
    console.log(`Average improvement: ${report.summary.avg_improvement_pct.toFixed(1)}%`);
    console.log(`P95 improvement: ${report.summary.p95_improvement_pct.toFixed(1)}%`);
    ...
    // Capture baseline
    const phase = args.includes("--post-optimization") ? "post-optimization" : "pre-optimization";
    ...
    try {
      ...
      console.log(`Average execution: ${report.summary.avg_execution_time_ms.toFixed(2)}ms`);
      ...
      console.log(`Index adoption: ${(report.summary.index_adoption_rate * 100).toFixed(0)}%`);
      console.log(`\nReport saved: ${reportPath}`);
