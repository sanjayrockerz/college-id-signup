...
class DatasetFidelityValidator {
  ...
  private async validateConversationSizes(): Promise<void> {
    ...
    const smallGroup = sizes.filter((s) => s >= 3 && s <= 20).length / sizes.length;
    const largeGroup = sizes.filter((s) => s > 20).length / sizes.length;

    // Expected (from distribution-spec.json)
    const expectedOneToOne = 0.70;
    const expectedSmallGroup = 0.25;
    const expectedLargeGroup = 0.05;

    this.report.distributions.conversation_sizes.push(
      this.checkDeviation("1:1 conversations", expectedOneToOne, oneToOne, TOLERANCE_BANDS.normal),
      this.checkDeviation("Small groups (3-20)", expectedSmallGroup, smallGroup, TOLERANCE_BANDS.normal),
      this.checkDeviation("Large groups (>20)", expectedLargeGroup, largeGroup, TOLERANCE_BANDS.relaxed)
    );
  ...
  private async validateMessagesPerConversation(): Promise<void> {
    ...
    const messageCounts = conversations.map((c) => c._count.messages).sort((a, b) => a - b);
    ...
    this.report.distributions.messages_per_conversation.push(
      this.checkDeviation("Message count p50", expectedP50, p50, TOLERANCE_BANDS.normal),
      this.checkDeviation("Message count p95", expectedP95, p95, TOLERANCE_BANDS.relaxed),
      this.checkDeviation("Message count p99", expectedP99, p99, TOLERANCE_BANDS.relaxed)
    );
  ...
  private async validateContentLength(): Promise<void> {
    ...
    const medianLength = this.percentile(lengths.sort((a, b) => a - b), 0.5);
    ...
    this.report.distributions.content_length.push(
      this.checkDeviation("Content length mean", expectedMean, avgLength, TOLERANCE_BANDS.normal),
      this.checkDeviation("Content length median", expectedMedian, medianLength, TOLERANCE_BANDS.normal)
    );
  ...
  private async validateMediaRatio(): Promise<void> {
    ...
    this.report.distributions.media_ratio.push(
      this.checkDeviation("Media message ratio", expectedRatio, actualRatio, TOLERANCE_BANDS.strict)
    );

    console.log(`  Media ratio: ${(actualRatio * 100).toFixed(1)}% (expected 15%)`);
  }
  ...
  private async validateDiurnalPattern(): Promise<void> {
    ...
    const peakAvg =
      peakHours.reduce((sum, h) => sum + hourlyDistribution[h], 0) / peakHours.length;
    const troughAvg =
      troughHours.reduce((sum, h) => sum + hourlyDistribution[h], 0) / troughHours.length;
    ...
    this.report.distributions.diurnal_pattern.push(
      this.checkDeviation("Peak/trough ratio", expectedRatio, peakToTroughRatio, TOLERANCE_BANDS.relaxed)
    );

    this.report.charts.hourly_message_distribution = hourlyDistribution;

    console.log(`  Peak/trough ratio: ${peakToTroughRatio.toFixed(2)}x (expected ~2.5x)`);
  }
  ...
  private async validateHeavyRooms(): Promise<void> {
    ...
    const expectedShare = 0.30; // 30%

    this.report.distributions.heavy_rooms.push(
      this.checkDeviation("Heavy room traffic share", expectedShare, trafficShare, TOLERANCE_BANDS.relaxed)
    );

    this.report.charts.heavy_room_traffic_share = trafficShare;

    // Verify at least some heavy rooms exist (>10k messages)
    const heavyRoomCount = conversations.filter((c) => c._count.messages > 10000).length;
    const heavyRoomPercentage = heavyRoomCount / conversations.length;

    this.report.distributions.heavy_rooms.push(
      this.checkDeviation("Heavy room percentage", 0.02, heavyRoomPercentage, TOLERANCE_BANDS.relaxed)
    );

    console.log(`  Top 2% traffic share: ${(trafficShare * 100).toFixed(1)}% (expected ~30%)`);
    console.log(
  ...
  private async validatePIIAbsence(): Promise<void> {
    ...
    for (const user of users) {
      ...
      if (user.email && !user.email.includes("synthetic_") && !user.email.includes("@example.local")) {
...
async function main() {
  ...
  if (configIndex === -1) {
    console.error("Usage: ts-node validate-dataset-fidelity.ts --config <report.json> [--tolerance 0.15]");
    process.exit(1);
  }

  const configPath = args[configIndex + 1];
  const tolerance = toleranceIndex !== -1 ? parseFloat(args[toleranceIndex + 1]) : TOLERANCE_BANDS.normal;
