...
class ProductionShapeSampler {
  ...
  private async sampleUsers(
    ...
    for (const user of users) {
      ...
      profileCompleteness[completenessBucket] =
        (profileCompleteness[completenessBucket] || 0) + 1;
  ...
  private async sampleMessages(
    ...
    for (const msg of messages) {
      ...
      if (previousTime) {
        const diffSeconds =
          (msg.createdAt.getTime() - previousTime.getTime()) / 1000;
        const arrivalBucket = this.bucket(diffSeconds, 60);
        interArrivalHistogram[arrivalBucket] =
          (interArrivalHistogram[arrivalBucket] || 0) + 1;
   ...
   */
  private async sampleReadReceipts(
    windowDays: number,
  ): Promise<ShapeMetrics["read_receipts"]> {
   ...
   */
  private async sampleAttachments(
    windowDays: number,
  ): Promise<ShapeMetrics["attachments"]> {
    ...
    for (const attachment of attachments) {
      typeDistribution[attachment.type] =
        (typeDistribution[attachment.type] || 0) + 1;
    ...
    return {
      ...
      per_message_attachment_ratio:
        messagesWithAttachments.size / Math.max(1, totalMessages),
    };
  }
  ...
  async sample(windowDays: number): Promise<ShapeMetrics> {
    console.log(
      `[ShapeSampler] Starting shape extraction (${windowDays} day window)`,
    );
...
async function main() {
  ...
  if (outputIndex === -1) {
    console.error(
      "Usage: ts-node production-sampler.ts --output <file.json> [--window-days N]",
    );
    process.exit(1);
  }
  ...
  if (!process.env.ANONYMIZATION_SALT) {
    ...
    console.error(
      "This script must run with proper anonymization configuration",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (
      combined.includes("@") ||
      combined.match(/\b[A-Z][a-z]+ [A-Z][a-z]+\b/)
    ) {
    ...
    console.log(
      "[Security] ✓ All tokens irreversible (HMAC with production salt)",
    );
    console.log("\n=== COMPLIANCE SUMMARY ===");
