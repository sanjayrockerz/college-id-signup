...
class DatasetQualityValidator {
  ...
  private async analyzeHeavyRooms() {
    ...
    const top1Messages = msgCounts
      .slice(0, top1Count)
      .reduce((sum, c) => sum + c.count, 0);
    const top5Messages = msgCounts
      .slice(0, top5Count)
      .reduce((sum, c) => sum + c.count, 0);
    ...
    const verdict: "PASS" | "FAIL" =
      top1Share >= 0.15 && top5Share >= 0.3 ? "PASS" : "FAIL";

    console.log(
      `     Top 1%: ${top1Count} convos with ${(top1Share * 100).toFixed(1)}% of messages`,
    );
    console.log(
      `     Top 5%: ${top5Count} convos with ${(top5Share * 100).toFixed(1)}% of messages`,
    );
  ...
  private async analyzePowerLawTail() {
    ...
    const percentile = (p: number) =>
      msgCounts[Math.floor((msgCounts.length - 1) * (p / 100))];
    ...
    console.log(
      `     Percentiles: p50=${p50}, p75=${p75}, p90=${p90}, p95=${p95}, p99=${p99}, max=${max}`,
    );
  ...
  private async validateTypeDistributions() {
    ...
    const verdict: "PASS" | "FAIL" =
      hasMultipleConvoTypes && hasMultipleMsgTypes ? "PASS" : "FAIL";

    console.log(`     Conversation types:`);
    convoDist.forEach((d) =>
      console.log(`       ${d.type}: ${d.count} (${d.percentage}%)`),
    );
  ...
  private async analyzeTemporalPatterns() {
    ...
    console.log(
      `     Diurnal variation: ${diurnalVariation.toFixed(2)}x (max/min)`,
    );
    console.log(`     Verdict: ${verdict}\n`);
  ...
  private async runIntegrityChecks() {
    ...
    const timestampVerdict: "PASS" | "FAIL" =
      outOfOrderCount === 0 ? "PASS" : "FAIL";
    ...
    const convosWithoutCreator = await this.prisma.$queryRaw<
      [{ count: bigint }]
    >`
      SELECT COUNT(*) as count FROM conversations WHERE "creatorId" IS NULL
    `.then((r) => Number(r[0].count));

    const completenessVerdict: "PASS" | "FAIL" =
      msgsWithoutSender === 0 && convosWithoutCreator === 0 ? "PASS" : "FAIL";

    console.log(`     PII Safety: ${realEmails} real emails (${piiVerdict})`);
    console.log(
      `     Referential integrity: ${orphanedMessages + orphanedConvoUsers + orphanedAttachments} orphaned records (${refIntegrityVerdict})`,
    );
    console.log(
      `     Timestamp ordering: ${outOfOrderCount} out-of-order (${timestampVerdict})`,
    );
    console.log(
      `     Data completeness: ${msgsWithoutSender + convosWithoutCreator} null FKs (${completenessVerdict})\n`,
    );
  ...
  private generateAssessment(
    ...
    if (integrity.referential_integrity.verdict === "FAIL") {
      criticalFailures.push(
        "Referential integrity violations - MUST fix constraints",
      );
    }
    if (heavyRooms.verdict === "FAIL") {
      warnings.push(
        "Insufficient heavy rooms - hot paths won't be adequately stressed",
      );
    }
  ...
  async validate(outputPath: string): Promise<void> {
    ...
    // Run tests
    const [heavyRooms, powerLaw, types, temporal, integrity] =
      await Promise.all([
    ...
    if (
      ...
      if (integrity.pii_safety.verdict === "FAIL") {
        recommendations.push(
          "⚠️ CRITICAL: Regenerate dataset with synthetic emails only",
        );
      }
      if (integrity.referential_integrity.verdict === "FAIL") {
        recommendations.push(
          "⚠️ CRITICAL: Fix foreign key constraints before testing",
        );
      }
      if (heavyRooms.verdict === "FAIL") {
        recommendations.push(
          "Decrease power-law alpha to create heavier tail (more huge rooms)",
        );
      }
      if (powerLaw.verdict === "FAIL") {
        recommendations.push(
          "Adjust distribution to create stronger p99/p50 ratio (>10x)",
        );
      }
    ...
    console.log(
      `Ready for Phase 2: ${assessment.ready_for_phase2 ? "YES ✓" : "NO ✗"}\n`,
    );
