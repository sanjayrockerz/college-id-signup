...
class HeavyRoomGenerator {
  ...
  private powerLaw(min: number, max: number, alpha: number): number {
    ...
    return Math.floor(
      Math.pow(minAlpha + u * (maxAlpha - minAlpha), 1 / (1 - alpha)),
    );
  }
  ...
  async generate(): Promise<void> {
    ...
    console.log(
      `[Step 2/4] Generating ${this.config.conversationCount} conversations...`,
    );
    ...
    for (let i = 0; i < messagesPerConvo.length; i++) {
      messagesPerConvo[i] = Math.max(
        1,
        Math.floor(messagesPerConvo[i] * scaleFactor),
      );
    }
    ...
    const top1Messages = messagesPerConvo
      .slice(0, top1Percent)
      .reduce((a, b) => a + b, 0);
    const top5Messages = messagesPerConvo
      .slice(0, top5Percent)
      .reduce((a, b) => a + b, 0);
    ...
    console.log(
      `    Top 1% (${top1Percent} convos): ${((top1Messages / totalMessages) * 100).toFixed(1)}% of messages`,
    );
    console.log(
      `    Top 5% (${top5Percent} convos): ${((top5Messages / totalMessages) * 100).toFixed(1)}% of messages`,
    );
    console.log(
      `    p50: ${messagesPerConvo[Math.floor(messagesPerConvo.length / 2)]}`,
    );
    console.log(
      `    p95: ${messagesPerConvo[Math.floor(messagesPerConvo.length * 0.05)]}`,
    );
    console.log(
      `    p99: ${messagesPerConvo[Math.floor(messagesPerConvo.length * 0.01)]}`,
    );
    ...
    const conversations = messagesPerConvo.map((msgCount, i) => {
      ...
      return {
        name: `Heavy Room ${i}`,
        type: (isDM ? "DIRECT_MESSAGE" : "GROUP_CHAT") as
          | "DIRECT_MESSAGE"
          | "GROUP_CHAT"
          | "CHANNEL",
        creatorId,
    ...
    for (let i = 0; i < conversations.length; i += batchSize) {
      ...
      if ((i + batchSize) % 2000 === 0) {
        console.log(
          `  Progress: ${i + batchSize}/${this.config.conversationCount}`,
        );
      }
    }
    ...
    for (let i = 0; i < this.conversationIds.length; i++) {
      ...
      const participantCount =
        convo.type === "DIRECT_MESSAGE" ? 1 : Math.floor(Math.random() * 9) + 2;
      ...
      for (let j = 0; j < participantCount; j++) {
        ...
        if (userId !== convo.creatorId) {
          participants.push({
            ...
            userId,
            joinedAt: new Date(
              convo.createdAt.getTime() + Math.random() * 1000 * 60 * 60,
            ),
          });
        }
      }
    ...
    const participantsTime = ((Date.now() - startParticipants) / 1000).toFixed(
      1,
    );
    ...
    for (let i = 0; i < this.conversationIds.length; i++) {
      ...
      for (let j = 0; j < msgCount; j++) {
        const senderId =
          convoParticipants[
            Math.floor(Math.random() * convoParticipants.length)
          ];
        const messageType =
          Math.random() < 0.83
            ? "TEXT"
        ...
        messageBatch.push({
          ...
          type: messageType,
          createdAt: new Date(
            baseTime + j * 1000 * 60 + Math.random() * 1000 * 60,
          ),
        });
        ...
        if (messageBatch.length >= insertBatchSize) {
          ...
          if (messagesGenerated % 50000 === 0) {
            ...
            const rate = Math.floor(
              (messagesGenerated / (Date.now() - startMessages)) * 1000,
            );
            console.log(
              `  Progress: ${messagesGenerated.toLocaleString()}/${totalMessages.toLocaleString()} (${progress}%) - ${rate} msg/s - ${elapsed}s elapsed`,
            );
          }
        }
      }
