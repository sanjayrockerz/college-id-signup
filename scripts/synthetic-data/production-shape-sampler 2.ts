...
class ProductionShapeSampler {
  ...
  constructor() {
    ...
    console.log("[ShapeSampler] ⚠️  PRIVACY MODE: Only aggregate metrics will be exported");
  }
  ...
  private async sampleUsers(windowDays: number): Promise<ShapeMetrics["users"]> {
    ...
    console.log(`[Users] Sampled ${users.length} users (last ${windowDays} days)`);
    ...
    for (const user of users) {
      ...
      const fieldsCount = [user.bio, user.profileImageUrl].filter(Boolean).length;
      const key = fieldsCount === 1 ? "1_field" : fieldsCount === 2 ? "2_fields" : "0_fields";
  ...
  private async sampleConversations(windowDays: number): Promise<ShapeMetrics["conversations"]> {
    ...
    console.log(`[Conversations] Sampled ${conversations.length} conversations`);
  ...
  private async sampleMessages(windowDays: number): Promise<ShapeMetrics["messages"]> {
    ...
    const mean = contentLengths.reduce((sum, v) => sum + v, 0) / contentLengths.length;
