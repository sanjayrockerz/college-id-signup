...
class FidelityValidator {
  ...
  constructor(shapePath: string, tolerance: number) {
    ...
    console.log(
      `[Validator] Loading production shape metrics from ${shapePath}...`,
    );
  ...
  private chiSquare(
    ...
    const criticalValue =
      degreesOfFreedom === 1 ? 3.84 : degreesOfFreedom === 2 ? 5.99 : 7.81;
    const pValue = chiSq > criticalValue ? 0.01 : 0.1;
  ...
  private async testConversationTypes() {
    ...
    const prodNorm = this.normalizeDistribution(
      this.prodMetrics.conversations.type_distribution,
    );
    ...
    console.log(
      `[Test 1]   Chi-Square: ${statistic.toFixed(2)}, p-value: ${pValue.toFixed(3)}`,
    );
  ...
  private async testMessagesPerConversation() {
    ...
    const prodPercentiles =
      this.prodMetrics.conversations.messages_per_conversation_percentiles;
    ...
    const verdict: "PASS" | "FAIL" =
      maxError < this.tolerance ? "PASS" : "FAIL";

    console.log(`[Test 2]   KS Statistic: ${ks.toFixed(3)}`);
    console.log(
      `[Test 2]   Max Percentile Error: ${(maxError * 100).toFixed(1)}%`,
    );
  ...
  private async testMessageTypes() {
    ...
    const synthNorm = this.normalizeDistribution(synthDist);
    const prodNorm = this.normalizeDistribution(
      this.prodMetrics.messages.type_distribution,
    );
    ...
    console.log(
      `[Test 3]   Chi-Square: ${statistic.toFixed(2)}, p-value: ${pValue.toFixed(3)}`,
    );
  ...
  private async testContentLength() {
    ...
    const verdict: "PASS" | "FAIL" =
      meanError < this.tolerance && medianError < this.tolerance
        ? "PASS"
        : "FAIL";
  ...
  async validate(outputPath: string): Promise<void> {
    ...
    const failedTests = Object.values(tests).filter(
      (t) => t.verdict === "FAIL",
    ).length;
    ...
    if (tests.messages_per_conversation.verdict === "FAIL") {
      recommendations.push(
        "Adjust power-law alpha in generator to better match production skew",
      );
    }
