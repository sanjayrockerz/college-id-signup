...
class PerformanceValidator {
  ...
  private async sampleConversations(): Promise<void> {
    console.log(`[Sampling] Selecting ${this.sampleSize} representative conversations...\n`);

    // Get conversation IDs with message counts
    const conversations = await this.prisma.$queryRaw<Array<{ id: string; msg_count: bigint }>>`
    ...
    return [
      {
        ...
        description: "Fetch 20 most recent messages in a conversation (hot path)",
        sql: `
      ...
      },
      {
        ...
        sql: `
          ...
          WHERE m."conversationId" = $1
            AND m."createdAt" > $2
        `,
        params: [this.conversationIds[1], new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)],
        expectation: {
          index_used: "idx_messages_conversation_created",
          no_sort: false,
          scan_type: "Index Scan",
      ...
      },
      {
        ...
        description: "Fetch 50 most recent messages from user's conversations (feed query)",
        sql: `
   ...
   */
  private async executeExplainAnalyze(query: string, params?: any[]): Promise<any> {
    ...
    if (params) {
      params.forEach((param, i) => {
        const paramValue = Array.isArray(param)
          ...
          : param;
        explainQuery = explainQuery.replace(new RegExp(`\\$${i + 1}`, "g"), String(paramValue));
      });
    }
   ...
   * Analyze query plan
   */
  private analyzePlan(plan: any, expectation: QueryTest["expectation"]): PlanAnalysis {
    ...
    const checkNode = (node: any): void => {
      ...
      if (nodeType.includes("Index Scan") || nodeType.includes("Index Only Scan")) {
      ...
      if (nodeType === "Sort") {
        ...
        analysis.warnings.push("SORT operation present - index not providing order");
      }
    ...
    if (!analysis.uses_index && expectation.index_used) {
      analysis.warnings.push(`Expected index '${expectation.index_used}' not used`);
    }

    if (analysis.has_sort && expectation.no_sort) {
      analysis.warnings.push("Unexpected Sort operation - index should provide order");
    }

    // Check buffer efficiency
    if (analysis.buffers) {
      const totalBuffers = analysis.buffers.shared_hit + analysis.buffers.shared_read;
      const hitRatio = totalBuffers > 0 ? (analysis.buffers.shared_hit / totalBuffers) * 100 : 0;
  ...
  async runTests(): Promise<PerformanceReport> {
    ...
    console.log(`[Testing] Running ${testQueries.length} query performance tests...\n`);
    ...
    for (let i = 0; i < testQueries.length; i++) {
      ...
      try {
        ...
        console.log(`  Index: ${analysis.uses_index ? `✓ ${analysis.index_name}` : "✗ Not used"}`);
        console.log(`  Sort: ${analysis.has_sort ? "✗ Present" : "✓ Absent"}`);
        console.log(`  Verdict: ${verdict === "PASS" ? "✓ PASS" : verdict === "FAIL" ? "✗ FAIL" : "⚠ WARN"}`);
    ...
    if (failed > 0) {
      recommendations.push("❌ CRITICAL: Some queries not using indexes - review and create missing indexes");
    }

    if (results.some((r) => r.analysis.has_sort && r.test.expectation.no_sort)) {
    ...
    if (results.some((r) => r.issues.includes("Sequential scan detected"))) {
      recommendations.push("⚠ Sequential scans detected - check index selectivity and statistics");
    }
