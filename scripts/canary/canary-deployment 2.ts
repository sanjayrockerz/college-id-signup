...
export class CanaryDeployment {
  ...
  constructor() {
    ...
    this.featureFlagService = process.env.FEATURE_FLAG_URL || 'http://localhost:3000/api/admin/feature-flags';
  }
  ...
  async run(config: CanaryConfig): Promise<CanaryResult> {
    ...
    try {
      ...
      for (let i = 0; i < config.stages.length; i++) {
        ...
        console.log(`Traffic: ${stage.trafficPercent}% | Duration: ${stage.durationMinutes}m`);
  ...
  private async captureMetrics(): Promise<CanaryMetrics> {
    const [
      ...
      this.queryPrometheus('rate(http_requests_total{status=~"5.."}[5m]) / rate(http_requests_total[5m])'),
      this.queryPrometheus('histogram_quantile(0.95, rate(http_request_duration_ms_bucket[5m]))'),
      this.queryPrometheus('db_pool_saturation'),
      this.queryPrometheus('db_tx_queue_wait_ms'),
      this.queryPrometheusString('circuit_breaker_state{component="read_replica"}'),
      this.queryPrometheus('rate(cache_operation_total{result="hit"}[5m]) / rate(cache_operation_total[5m])'),
    ]);
  ...
  private printMetrics(label: string, metrics: CanaryMetrics): void {
    ...
    console.log(`  Pool Saturation: ${(metrics.poolSaturation * 100).toFixed(1)}%`);
  ...
  messageHistory: {
    ...
    healthChecks: [
      ...
      },
      {
        ...
        expectedCondition: (value: number) => value < 0.80, // <80%
      },
    ],
  } as CanaryConfig,
  ...
  conversationList: {
    ...
    healthChecks: [
      ...
      },
      {
        name: 'pool_saturation',
        query: 'db_pool_saturation',
        expectedCondition: (value: number) => value < 0.80,
      },
    ],
  } as CanaryConfig,
};
...
async function main() {
  ...
  const config = Phase2CanaryConfigs[configName as keyof typeof Phase2CanaryConfigs];
