...
router.post('/signup', async (req, res) => {
  try {
    ...
    // Create the user with chat-enabled features
    const newUser = await prisma.user.create({
      data: {
        email: email.toLowerCase(),
        username: username.toLowerCase(),
        ...
        weeklyPushesUsed: 0,
        lastWeeklyReset: new Date(),
        allowDirectMessages: true, // Enable chat features
        showOnlineStatus: true,     // Enable online status for chat
        profileVisibility: 'PUBLIC'
      }
    });
    ...
    // Return user data (without password) with chat-ready status
    const { password: _, ...userWithoutPassword } = newUser;

    res.status(201).json({
      success: true,
      message: 'User signup successful - chat features enabled',
      data: {
        user: {
          ...userWithoutPassword,
          chatEnabled: true,
          canReceiveMessages: newUser.allowDirectMessages,
          isOnline: true
        },
        token
      }
