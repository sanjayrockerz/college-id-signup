...
export class UploadService {
  ...
  async processFiles(files: Express.Multer.File[]): Promise<{ validFiles: Express.Multer.File[]; errors: string[] }> {
  ...
  validateFile(file: Express.Multer.File): { isValid: boolean; errors: string[] } {
    ...
    if (!allowedMimeTypes.includes(file.mimetype)) {
      errors.push(`Invalid file type: ${file.mimetype}. Only images (JPEG, PNG, WebP) are allowed.`);
    }

    // Check file size
    if (file.size > maxFileSize) {
      errors.push(`File too large: ${file.size} bytes. Maximum allowed: ${maxFileSize} bytes.`);
    }
  ...
  async extractTextFromImage(imagePath: string): Promise<ExtractedData> {
    try {
      ...
      for (const line of lines) {
        ...
        if (cleanLine.toLowerCase().includes('student') || 
            cleanLine.toLowerCase().includes('name:') ||
            (cleanLine.length > 5 && cleanLine.length < 50 && /^[A-Za-z\s.]+$/.test(cleanLine))) {
          if (!studentName && !cleanLine.toLowerCase().includes('college') && 
        ...
        if (cleanLine.toLowerCase().includes('college') || 
            cleanLine.toLowerCase().includes('university') ||
            cleanLine.toLowerCase().includes('institute') ||
  ...
  private calculateConfidence(studentName: string, collegeName: string): number {
    let confidence = 0;
    
    if (studentName && studentName !== 'Not found' && studentName !== 'OCR failed') {
      confidence += 50;
    }
    
    if (collegeName && collegeName !== 'Not found' && collegeName !== 'OCR failed') {
      confidence += 50;
    }
