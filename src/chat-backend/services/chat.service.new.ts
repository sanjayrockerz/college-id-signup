...
import { ChatRepository, CreateConversationDto, SendMessageDto, GetMessagesDto } from '../repositories/chat.repository';
...
export interface ChatServiceInterface {
  ...
  getUserConversations(userId: string, limit?: number, cursor?: string): Promise<any>;
  sendMessage(conversationId: string, userId: string, data: SendMessageDto): Promise<any>;
  getMessages(conversationId: string, userId: string, options?: GetMessagesDto): Promise<any>;
  markMessagesAsRead(conversationId: string, userId: string, messageIds: string[]): Promise<any>;
...
export class ChatService implements ChatServiceInterface {
  ...
  async createConversation(userId: string, data: CreateConversationDto) {
    try {
      ...
      if (data.type === 'DIRECT') {
        const existingConversation = await this.chatRepository.findOrCreateDirectMessage(
          userId, 
      ...
      const conversation = await this.chatRepository.createConversation(userId, data);
      ...
      throw new BadRequestException(`Failed to create conversation: ${getErrorMessage(error)}`);
    }
  }
  ...
  async getUserConversations(userId: string, limit: number = 20, cursor?: string) {
    try {
      const result = await this.chatRepository.getUserConversations(userId, limit, cursor);
      ...
      throw new BadRequestException(`Failed to get conversations: ${getErrorMessage(error)}`);
    }
  }
  ...
  async sendMessage(conversationId: string, userId: string, data: SendMessageDto) {
    try {
      ...
      const message = await this.chatRepository.sendMessage(conversationId, userId, data);
   ...
   */
  async getMessages(conversationId: string, userId: string, options: GetMessagesDto = {}) {
    try {
      const result = await this.chatRepository.getMessages(conversationId, userId, options);
  ...
  async markMessagesAsRead(conversationId: string, userId: string, messageIds: string[]) {
    try {
      ...
      const result = await this.chatRepository.markMessagesAsRead(conversationId, userId, messageIds);
      ...
      throw new BadRequestException(`Failed to mark messages as read: ${errorMessage}`);
    }
  }
  ...
  async getUnreadCount(userId: string): Promise<number> {
    try {
      ...
      throw new BadRequestException(`Failed to get unread count: ${getErrorMessage(error)}`);
    }
  }
  ...
  async findOrCreateDirectMessage(user1Id: string, user2Id: string) {
    try {
      ...
      const conversation = await this.chatRepository.findOrCreateDirectMessage(user1Id, user2Id);
      ...
      throw new BadRequestException(`Failed to create direct message: ${getErrorMessage(error)}`);
    }
  }
   ...
   */
  async addUserToConversation(conversationId: string, userId: string, addedByUserId: string) {
    try {
      ...
      throw new BadRequestException(`Failed to add user to conversation: ${errorMessage}`);
    }
  }
  ...
  async getConversationDetails(conversationId: string, userId: string) {
    try {
      ...
      const result = await this.chatRepository.getUserConversations(userId, 100);
      ...
      throw new BadRequestException(`Failed to get conversation details: ${getErrorMessage(error)}`);
    }
  }
  ...
  async searchMessages(conversationId: string, userId: string, query: string, limit: number = 20) {
    try {
      ...
      throw new BadRequestException(`Failed to search messages: ${getErrorMessage(error)}`);
    }
  }
  ...
  async getConversationStats(conversationId: string, userId: string) {
    try {
      ...
      throw new BadRequestException(`Failed to get conversation stats: ${getErrorMessage(error)}`);
    }
  }
}
