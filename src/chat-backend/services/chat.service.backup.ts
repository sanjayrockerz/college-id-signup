...
export class ChatService implements ChatServiceInterface {
  ...
  async createConversation(userId: string, data: CreateConversationDto) {
    try {
      ...
      throw new BadRequestException(`Failed to create conversation: ${errorMessage}`);
    }
  }
  ...
  async addUserToConversation(conversationId: string, userId: string, addedByUserId: string) {
    try {
      ...
      if (error.message.includes('Only conversation admins')) {
        throw new ForbiddenException('Only conversation admins can add users');
      }
      if (error.message.includes('already in this conversation')) {
        throw new BadRequestException('User is already in this conversation');
      }
      throw new BadRequestException(`Failed to add user to conversation: ${error.message}`);
    }
  }
  ...
  async getConversationDetails(conversationId: string, userId: string) {
    try {
      ...
      throw new BadRequestException(`Failed to get conversation details: ${error.message}`);
    }
  }
  ...
  async getConversationStats(conversationId: string, userId: string) {
    try {
      ...
      throw new BadRequestException(`Failed to get conversation stats: ${error.message}`);
    }
  }
}
