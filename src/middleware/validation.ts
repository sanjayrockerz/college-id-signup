 ...
 */
function sendValidationError(res: Response, message: string, details?: any): Response {
 ...
 */
export function validateConversationId(req: Request, res: Response, next: NextFunction): void {
 ...
 */
export function validateMessageContent(req: Request, res: Response, next: NextFunction): void {
  ...
  if (content.length > SIZE_LIMITS.MESSAGE_CONTENT) {
    sendValidationError(res, `Message content is too long (max ${SIZE_LIMITS.MESSAGE_CONTENT} characters)`, {
 ...
 */
export function validateConversationCreate(req: Request, res: Response, next: NextFunction): void {
  ...
  if (title !== undefined) {
    ...
    if (title.length > SIZE_LIMITS.CONVERSATION_TITLE) {
      sendValidationError(res, `title is too long (max ${SIZE_LIMITS.CONVERSATION_TITLE} characters)`);
      return;
    }
  }
  ...
  if (description !== undefined) {
    ...
    if (description.length > SIZE_LIMITS.CONVERSATION_DESCRIPTION) {
      sendValidationError(res, `description is too long (max ${SIZE_LIMITS.CONVERSATION_DESCRIPTION} characters)`);
      return;
    }
  }
 ...
 */
export function validateMessageIds(req: Request, res: Response, next: NextFunction): void {
 ...
 */
export function validateIdCardVerification(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!collegeName || typeof collegeName !== 'string' || collegeName.trim().length === 0) {
    sendValidationError(res, 'collegeName is required and must be a non-empty string');
    return;
  }
  ...
  if (!studentIdNumber || typeof studentIdNumber !== 'string' || studentIdNumber.trim().length === 0) {
    sendValidationError(res, 'studentIdNumber is required and must be a non-empty string');
    return;
  }
  
  if (studentIdNumber.length > 100) {
    sendValidationError(res, 'studentIdNumber is too long (max 100 characters)');
    return;
  }
