...
export class UserController {
  ...
  @Put('me')
  async updateCurrentUser(@Body() updateUserDto: UpdateUserDto, @Request() req: any): Promise<UserResponseDto> {
    const userId = req.user?.id || 'temp-user-id';
  ...
  @Get('me/limits')
  async getUserLimits(@Request() req: any): Promise<{ canPostAnonymous: boolean; canPush: boolean; }> {
    const userId = req.user?.id || 'temp-user-id';
    const canPostAnonymous = await this.userService.checkAnonymousPostLimit(userId);
