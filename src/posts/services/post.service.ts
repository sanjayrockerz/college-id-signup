...
export class PostService {
  ...
  async createPost(userId: string, createPostDto: CreatePostDto): Promise<PostResponseDto> {
    ...
    if (createPostDto.isAnonymous) {
      ...
      if (anonymousPostsToday >= 2) {
        throw new BadRequestException('Daily anonymous post limit exceeded (2 posts per day)');
      }
    }
    ...
    const mockPost: PostResponseDto = {
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      author: createPostDto.isAnonymous ? undefined : {
        id: userId,
        username: 'temp-user',
        firstName: 'John',
        lastName: 'Doe',
  ...
  async getPostById(postId: string, userId?: string): Promise<PostResponseDto | null> {
    // TODO: Implement with actual repository
    return null;
  }
  ...
  async getUserPosts(userId: string, requesterId?: string, limit = 10, cursor?: string): Promise<PostResponseDto[]> {
