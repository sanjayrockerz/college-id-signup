...
export class DatabaseHealthService {
  ...
  async checkDatabaseConnection(): Promise<{
    ...
    try {
      ...
      // Check if using mock client via global config
      if (isPrismaMockMode()) {
        return {
          status: "mock",
          ...
          details: {
            type: "mock",
            mode: activePrismaClientMode,
            environment: process.env.NODE_ENV || "development",
            prismaClientMode: process.env.PRISMA_CLIENT_MODE || "auto",
      ...
      try {
        if ("$queryRaw" in this.prisma) {
          queryResult = await (this.prisma as any)
            .$queryRaw`SELECT NOW() as current_time`;
        }
      } catch (queryError) {
  ...
  async testDatabaseOperations(): Promise<{
    ...
    try {
      ...
      try {
        ...
        errors.push(
          `Connection count failed: ${error?.message || "Unknown error"}`,
        );
      }

      this.logger.log(
        `Database operations test completed. ${operations.length} operations, ${errors.length} errors`,
      );
