...
export class IdCardController {
  ...
  async uploadIdCard(
    ...
    if (file.size > 10 * 1024 * 1024) {
      throw new PayloadTooLargeException(
        "File too large for mobile processing",
      );
    }
    ...
    const optimizedImage =
      await this.mobileOptimizationService.optimizeImageForMobile(file.buffer, {
  ...
  async getMobileVerificationHistory(
    ...
    const optimizedHistory =
      this.mobileOptimizationService.optimizeArrayForMobile(
        history,
        1, // page number
  ...
  private detectNetworkQuality(
    ...
    if (userAgent.includes("mobile") && !userAgent.includes("wifi"))
      return "fair";
