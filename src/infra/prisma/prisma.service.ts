...
export class PrismaService implements OnModuleInit, OnModuleDestroy {
  ...
  get coolnessRating() {
    return prisma.coolnessRating;
  }
  get push() {
    return prisma.push;
  }
  get postView() {
    return prisma.postView;
  }
  ...
  get conversationUser() {
    return prisma.conversationUser;
  }
  get message() {
    return prisma.message;
  }
  get messageRead() {
    return prisma.messageRead;
  }
  get attachment() {
  ...
  get $executeRaw() {
    return prisma.$executeRaw.bind(prisma);
  }
  get $queryRawUnsafe() {
  ...
  async onModuleInit() {
    ...
    if (isPrismaMockMode()) {
      this.logger.warn(
        "Prisma mock mode enabled - database operations run against in-memory stubs.",
      );
    } else {
      this.logger.log(
  ...
  async onModuleDestroy() {
    ...
    this.logger.log(
      "PrismaService destroyed (singleton connection remains active)",
    );
  }
  ...
  async healthCheck(): Promise<{
    ...
    return {
      ...
      latency: health.latency
        ? parseInt(health.latency.replace("ms", ""))
        : undefined,
    };
  }
